...
class Job:
    ...
    def __init__(
        ...
        **Raises:**
        ...
        if sys not in SCHEDULER:
            print(
                "Unknown scheduler implementation, must be one of: "
                + str(SCHEDULER.keys())
            )
        ...
        if not self.jobscript:
            self.jobscript = os.path.join(os.getcwd(), jobname + ".sh")
        self._init_tpl()
        self.header_dict["job_name"] = self.jobname
        self.header_dict["job_shell"] = "#!/bin/sh"
        if header_dict:
            self.header_dict.update(header_dict)
    ...
    def _init_tpl(self):
        ...
        if type(self.tpl) is str:
            ...
            raise Exception(
                "unknown type of job template, must be either "
    ...
    def submit(self, write=False):
        ...
        output = subprocess.Popen(
            self.batch_command.split() + [self.jobscript],
        ...
        if not stderr:
            ...
            for entry in self.control:
                logging.debug(
                    "control: " + str(entry) + ": " + str(self.control[entry])
                )
        else:
    ...
    def get_log(self):
        # basic info
        job_log = {"jobid": self.jobid, "jobscript": self.jobscript}
        # add some info from control command
        for ctr in self.control:
            if ctr in self.ctr_list:
                job_log[ctr] = self.control[ctr]
        return job_log
...
class Scheduler:
    ...
    def __init__(self, sys, name="", tpl=None, logfile="", job_list=[], header_dict={}):
    ...
    def create_job(self, jobname, jobscript, commands="", header_dict={}, write=True):
    ...
    def update_job_list(self, job):
        ...
        if job in self.job_list:
            self.job_list = [job if job == x else x for x in self.job_list]
        else:
            self.add_job(job)
    ...
    def get_job_list(self, filters=[]):
        ...
        if filters:
            job_list = [
                job
                for job in self.job_list
                if any(filter in job.jobname for filter in filters)
            ]
        return job_list
    ...
    def log_jobs_acct(self, filters=None):
        ...
        **Arguments:**
        ...
        for i in invert:
            ...
            if counter[i] > 0 or i >= 0:
                logging.log(LOGLEV[i], str(counter[i]) + " jobs " + invert[i])
